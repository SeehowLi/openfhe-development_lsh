...
void testSlots2Coeffs() {
    ...
    std::cout << "Precompute Ready!" << std::endl;
    ...
    std::vector<std::complex<double>> vec={
        ...
        std::complex<double>(0.5, 0.), std::complex<double>(0.0, 0.),
        std::complex<double>(0., 0.), std::complex<double>(0.0, 0.),
        std::complex<double>(0., 0.), std::complex<double>(0.0, 0.)
    };
    ...
    // cryptoContext->EvalLinearTransformPrecomputeForLevel(numSlots, 0, {2,2}, {0,0});
    ...
    // ciph = cryptoContext->EvalStC(ciph);
    ...
    for(size_t i=numSlots; i<xx1.GetLength(); i+=ringDim/numSlots) {
        std::cout << i << " ";
